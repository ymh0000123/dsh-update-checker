...
function classifyPnpmFailure(run) {
  ...
  // 2. pnpm installed but skipped the build, because the allowlist entry is
  //    missing, `false`, or still pnpm's "set this to true or false" placeholder.
  //    Left alone this yields a package with no build output at all.
  if (/Ignored build scripts:/.test(all)) {
    const ignored = /Ignored build scripts:\s*([^\r\n│]+)/.exec(all);
    const subject = ignored ? ignored[1].trim().replace(/[.\s]+$/, '') : name;
    return {
      kind: 'ignored-build-scripts',
      detail,
      message:
        name + ' 的构建脚本被跳过了（未获授权），装出来会缺少编译产物——同样是 allowBuilds 白名单问题，不是网络问题。'
        + '该条目当前不是 true（可能是 false，或 pnpm 写下的 “set this to true or false” 占位符）。'
        + '需要在 pnpm-workspace.yaml 的 allowBuilds 把它设为 true：'
        + '\n  ' + subject + ': true',
    };
  }
  ...
  // 5. pnpm itself is missing — nothing to do with the package being updated.
  if (/is not recognized as an internal or external command|command not found|not found: pnpm|ENOENT[^\n]*pnpm/i.test(all)) {
    return {
      kind: 'pnpm-missing',
      detail,
      message: '找不到 pnpm 命令。更新需要 pnpm 在 PATH 上可用（profile 本身就是 pnpm 装的），装好 pnpm 后重试。',
    };
  }

  // 6. The lockfile is out of sync with the manifest.
